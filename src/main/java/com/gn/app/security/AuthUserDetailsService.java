...
public class AuthUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (user != null) {
            ...
            springUser = new org.springframework.security.core.userdetails.User("empty",
                    ...
                    true,
                    false,
                    getAuthorities(Roles.ROLE_ADMIN)
            );
            return springUser;
        }
    }
    ...
    public List<GrantedAuthority> getAuthorities(Roles role) {
        ...
        if (role == Roles.ROLE_ADMIN) {
            authList.add(new SimpleGrantedAuthority(Roles.ROLE_ADMIN.name()));
        } else if (role ==Roles.ROLE_USER) {
            authList.add(new SimpleGrantedAuthority(Roles.ROLE_USER.name()));
        }
